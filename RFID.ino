//The RFId tag Acess Control Project
...
String accessGranted [2] = {"11338410100", "2201814115"};  //RFID serial numbers to grant access to
...
void checkAccess (String temp)    //Function to check if an identified tag is registered to allow access
{
  ...
  for (int i=0; i <= (accessGrantedSize-1); i++)    //Runs through all tag ID numbers registered in the array
  {
    ...
    {
      Serial.println ("Access Granted" +temp);
      granted = true;
  ...
  {
    Serial.print ("Access Denied" +temp);
